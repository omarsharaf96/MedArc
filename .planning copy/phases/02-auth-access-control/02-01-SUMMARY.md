...
plan: 01
subsystem: auth
tags: [argon2id, password-auth, session-management, tauri-commands, rusqlite, rbac]

# Dependency graph
requires:
  - phase: 01-desktop-shell-encrypted-database
    provides: "SQLCipher encrypted database, Tauri app shell, migration framework, error types"
provides:
...
affects: [02-02, 02-03, 02-04, 02-05, 03-patient-registration, 04-clinical-documentation]

# Tech tracking
tech-stack:
  added: [password-auth 1.x (argon2), rand 0.8]
  patterns: [session-state-machine, managed-tauri-state, bootstrap-first-user, account-lockout]
...
key-decisions:
  - "SessionInfo struct lives in auth::session module (single source of truth for session state representation)"
...
patterns-established:
  - "Session state machine pattern: Mutex<SessionState> with typed enum transitions"
  - "Bootstrap auth: first-run allows unauthenticated user creation, subsequent requires SystemAdmin"
  - "Configurable settings via app_settings table with sensible defaults seeded in migration"
  - "Password validation at both command level and hash function level (defense in depth)"
