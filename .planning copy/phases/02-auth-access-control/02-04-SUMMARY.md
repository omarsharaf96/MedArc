...
plan: 04
subsystem: auth-ui
tags: [react, tailwind, useAuth, useIdleTimer, login, mfa, lock-screen, tauri-invoke]

# Dependency graph
requires:
  - phase: 02-auth-access-control
    provides: "Auth Tauri commands (register_user, login, logout, session management), RBAC engine, break-glass commands"
provides:
...
affects: [02-05, 03-patient-registration, 04-clinical-documentation]

# Tech tracking
tech-stack:
  added: []
  patterns: [auth-gate-pattern, idle-timer-hook, auth-state-hook, lock-screen-overlay]
...
key-decisions:
  - "Used base64 img tag for QR code display instead of qrcode.react (backend already returns qrBase64, no extra dependency needed)"
  - "break_glass invoke wrapper includes password param matching actual Rust command signature (plan interface spec omitted it)"
...
patterns-established:
  - "Auth gate pattern: App.tsx conditionally renders login/register/mfa/main based on useAuth state"
  - "useAuth hook pattern: centralized auth state management with login/register/logout/unlock/mfa functions"
  - "Idle timer pattern: window event listeners with debounced backend refresh and timeout-based auto-lock"
  - "Auth component pattern: form components receive callbacks and state via props (no direct hook coupling)"
